...
import { 
  TrendingUp, ArrowLeft, BookOpen, Clock, Users, Play, 
...
export default function InvestingBasicsPage() {
  ...
  const LESSONS = [
    ...
    { id: 2, title: t.trackPage.understandingCashFlow, duration: "12 min", completed: false, unlocked: true, current: true },
    { id: 3, title: t.trackPage.revenueVsProfit, duration: "15 min", completed: false, unlocked: true },
    { id: 4, title: t.trackPage.financialStatements101, duration: "10 min", completed: false, unlocked: true },
    { id: 5, title: t.trackPage.assetsAndLiabilities, duration: "8 min", completed: false, unlocked: false },
    { id: 6, title: t.trackPage.buildingBudget, duration: "14 min", completed: false, unlocked: false },
    { id: 7, title: t.trackPage.emergencyFunds, duration: "12 min", completed: false, unlocked: false },
    { id: 8, title: t.trackPage.debtManagement, duration: "10 min", completed: false, unlocked: false },
    { id: 9, title: t.trackPage.creditScores, duration: "8 min", completed: false, unlocked: false },
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="mb-12">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white md:text-4xl">{t.trackPage.investingBasics}</h1>
                <p className="text-slate-400 mt-1">{t.trackPage.investingBasicsDesc}</p>
              </div>
            </div>
            
            <div className="flex flex-wrap items-center gap-6 text-sm text-slate-400 mt-6">
              <div className="flex items-center gap-2"><BookOpen className="h-4 w-4" />10 {t.trackPage.lessonsCount}</div>
              <div className="flex items-center gap-2"><Clock className="h-4 w-4" />1.5 {t.trackPage.hoursTotal}</div>
              <div className="flex items-center gap-2"><Users className="h-4 w-4" />1,890 {t.trackPage.students}</div>
            </div>

            <div className="mt-6">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="text-slate-400">{t.trackPage.yourProgress}</span>
                <span className="text-blue-400 font-medium">{completedCount}/{LESSONS.length} {t.trackPage.completed}</span>
              </div>
              <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
                <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.8, delay: 0.3 }} className="h-full rounded-full bg-gradient-to-r from-blue-500 to-blue-400" />
              </div>
            </div>
          </motion.div>
                  ...
                  {LESSONS.map((lesson, idx) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 rounded-xl px-3 py-3 text-left transition-all ${
                        selectedLesson.id === lesson.id ? "bg-blue-500/20 border border-blue-500/30" : lesson.unlocked ? "hover:bg-white/5" : "opacity-50 cursor-not-allowed"
                      }`}
                    >
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                        lesson.completed ? "bg-blue-500/20 text-blue-400" : lesson.unlocked ? "bg-white/10 text-slate-400" : "bg-white/5 text-slate-600"
                      }`}>
                      ...
                      {lesson.current && <span className="text-[10px] font-bold text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-full">{t.trackPage.current}</span>}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="lg:col-span-2">
              <div className="glass-card rounded-2xl overflow-hidden">
                <div className="relative aspect-video bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center">
                  ...
                  <div className="group relative flex h-20 w-20 cursor-pointer items-center justify-center rounded-full bg-blue-500/10 transition-all hover:scale-110 hover:bg-blue-500/20">
                    <div className="absolute inset-0 animate-ping rounded-full bg-blue-400/20"></div>
                    <Play className="h-8 w-8 fill-blue-500 text-blue-500 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </div>
              ...
              <div className="glass-card rounded-2xl p-6 mt-6 bg-white/[0.03] border border-white/10">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Link
                    href="/learn/investing-basics/test"
