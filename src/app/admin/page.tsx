        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-bold text-white">{a.portalTitle}</h1>
              </div>
              <p className="text-sm text-slate-400 mt-0.5">
                {a.welcomeBack.replace("{name}", user.name)}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 rounded-full bg-green-500/10 border border-green-500/20 px-3 py-1.5">
              ...
              <span className="text-xs font-medium text-green-400">{a.live}</span>
            </div>
            <div className="flex items-center gap-2 rounded-full bg-blue-500/10 border border-blue-500/20 px-4 py-2">
              <Crown className="h-4 w-4 text-blue-400" />
              <span className="text-sm font-medium text-blue-400">{a.superAdmin}</span>
            </div>
          </div>
        </div>
              ...
              {[
                {
                  ...
                  icon: Users,
                  color: "blue",
                  sub: a.thisWeek.replace("{n}", String(analytics.thisWeekSignups)),
                },
                {
                  label: a.totalVisits,
                  ...
                  icon: Globe,
                  color: "green",
                  sub: a.today.replace("{n}", String(analytics.todayVisits)),
                },
                {
                  label: a.totalSignups,
                  value: analytics.totalSignups,
                  icon: UserPlus,
                  color: "purple",
                  sub: a.today.replace("{n}", String(analytics.todaySignups)),
                },
                {
                  label: a.pageViews,
                  ...
                  icon: Eye,
                  color: "yellow",
                  sub: a.uniqueSessions.replace("{n}", String(analytics.uniqueSessions)),
                },
              ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  ...
                  {a.latestSignups}
                  <span className="text-xs text-slate-500 font-normal">{a.autoRefreshes}</span>
                </h3>
                <div className="space-y-3">
                  ...
                  {signups.length === 0 && (
                    <p className="text-sm text-slate-500 text-center py-4">{a.noSignupsYet}</p>
                  )}
                </div>
              </div>
                  ...
                  {users.slice(0, 6).map((u) => (
                    <div key={u.id} className="flex items-center justify-between py-2.5 border-b border-white/5 last:border-0">
                      ...
                      <div className="text-right">
                        ...
                        {u.isAdmin && (
                          <span className="block text-[10px] text-blue-400 font-medium">{a.roleAdmin}</span>
                        )}
                      </div>
                    </div>
                  ))}
                  {users.length === 0 && (
                    <p className="text-sm text-slate-500 text-center py-4">{a.noUsersYet}</p>
                  )}
                </div>
              </div>
            </div>
                  ...
                  return (
                    <div key={day.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                      <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 border border-white/10 rounded-lg px-2 py-1 text-[10px] text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                        {a.chartTooltipVisits
                          .replace("{date}", day.date.slice(5))
                          .replace("{visits}", String(day.count))
                          .replace("{signups}", String(signupDay?.count || 0))}
                      </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/10">
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colUser}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colEmail}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colJoined}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colLastActive}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colRole}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colStatus}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredUsers.map((u) => {
                      ...
                      <tr key={u.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-6 py-4">
                          {activity?.lastActiveAt ? (
                            ...
                            <span className="text-xs text-slate-500">{a.none}</span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          {u.isAdmin ? (
              ...
              {filteredUsers.length === 0 && (
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm text-slate-500">{a.noUsersFound}</p>
                </div>
              )}
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-white flex items-center gap-2">
                <Bell className="h-5 w-5 text-blue-400" />
                {a.allSignups}
                <span className="text-sm font-normal text-slate-500">{a.instantReg}</span>
              </h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <p className="text-sm text-slate-400 mb-1">{a.todayLabel}</p>
                <p className="text-3xl font-bold text-white">{analytics.todaySignups}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <p className="text-sm text-slate-400 mb-1">{a.thisWeekLabel}</p>
                <p className="text-3xl font-bold text-white">{analytics.thisWeekSignups}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <p className="text-sm text-slate-400 mb-1">{a.thisMonthLabel}</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/10">
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colPerson}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colEmail}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colSignedUp}</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">{a.colTimeAgo}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {signups.map((s) => (
              ...
              {signups.length === 0 && (
                <div className="py-12 text-center">
                  ...
                  <p className="text-sm text-slate-500">{a.noSignupsRecorded}</p>
                  <p className="text-xs text-slate-600 mt-1">{a.signupsAppearHere}</p>
                </div>
              )}
            </div>
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              <h3 className="text-lg font-semibold text-white mb-4">{a.signupsPerDay}</h3>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {[
                ...
                {
                  label: a.uniqueSessionsLabel,
                  value: analytics.uniqueSessions,
                  sub: a.basedOnSessionIds,
                },
                {
                  label: a.pageViews,
                  value: analytics.totalPageViews,
                  sub: a.mostPopular.replace(
                    "{page}",
                    analytics.topPages[0]?.page || a.none,
                  ),
                },
                {
                  label: a.thisWeekVisits,
                  value: analytics.thisWeekVisits,
                  sub: a.thisMonthVisits.replace(
                    "{n}",
                    String(analytics.thisMonthVisits),
                  ),
                },
              ].map((stat, i) => (
                <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <div className="space-y-3">
                {analytics.topPages.length > 0 ? analytics.topPages.map((page, i) => {
                  ...
                  return (
                    ...
                    <p className="text-sm text-slate-500 text-center py-4">{a.noPageViews}</p>
                  )}
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              <h3 className="text-lg font-semibold text-white mb-4">{a.visitsPerDay}</h3>
              <div className="flex items-end gap-1.5 h-28">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {[
                {
                  label: a.monthlyPlan,
                  subscribers: revenueStats.monthlySubscribers,
                  price: "$14.99/mo",
                  revenue: (revenueStats.monthlySubscribers * 14.99).toFixed(2),
                  color: "blue",
                },
                {
                  label: a.semiAnnualPlan,
                  subscribers: revenueStats.semiAnnualSubscribers,
                  price: "$10.00/mo",
                  revenue: (revenueStats.semiAnnualSubscribers * 10).toFixed(2),
                  color: "purple",
                },
                {
                  label: a.annualPlan,
                  subscribers: revenueStats.annualSubscribers,
                  price: "$7.50/mo",
                  revenue: (revenueStats.annualSubscribers * 7.5).toFixed(2),
                  color: "green",
                },
              ].map((plan, i) => (
                <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  ...
                  <p className="text-xs text-slate-500">{a.subscribers}</p>
                  <div className="mt-3 pt-3 border-t border-white/5">
                    <p className="text-sm text-slate-400">
                      {a.revenueCol}{" "}
                      <span className="text-white font-medium">${plan.revenue}</span>
                      {a.perMonth}
                    </p>
                  </div>
                </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <p className="text-sm text-slate-400 mb-1">{a.totalMRR}</p>
                  <p className="text-4xl font-bold text-white">${revenueStats.mrr}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">{a.estimatedARR}</p>
                  <p className="text-4xl font-bold text-white">${(parseFloat(revenueStats.mrr) * 12).toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">{a.totalPremiumUsers}</p>
                  <p className="text-4xl font-bold text-white">{revenueStats.premiumUsers}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">{a.conversionRate}</p>
