...
const Navigation = () => {
  ...
  const lightModeLabel =
    language === "ru" ? "Светлая тема" : language === "uz" ? "Yorug' rejim" : "Light mode";
  const darkModeLabel =
    language === "ru" ? "Тёмная тема" : language === "uz" ? "Qorong'i rejim" : "Dark mode";
  ...
  return (
    <>
      <StreakCelebration />
      <StreakCalendar open={streakCalendarOpen} onClose={() => setStreakCalendarOpen(false)} />
      ...
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div
            className={`flex items-center justify-between rounded-full border px-3 py-2.5 transition-all duration-300 sm:px-6 sm:py-3 ${
              scrolled
                ? "backdrop-blur-[16px] shadow-[var(--shadow-md)]"
                : "backdrop-blur-xl shadow-[var(--shadow-navbar)] glass-card"
            }`}
            style={
              scrolled
                ? { ...navBarStyle, backgroundColor: "var(--bg-navbar)" }
                : navBarStyle
            }
          >
            {/* Logo + social */}
            ...
            <div className="hidden items-center gap-3 md:flex">
              ...
              <Link
                href="/ai"
                className="flex items-center gap-1.5 rounded-full border border-amber-200/60 bg-amber-50/80 px-2.5 py-1 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-100 dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-300 dark:hover:bg-amber-500/15"
                title="Zinverts — used to ask AI questions"
              >
                <Image src="/icon-192.png" alt="" width={14} height={14} className="rounded" />
                {coins}
              </Link>

              {/* Streak badge — clickable, opens calendar */}
              {streak > 0 && (
                <button
                  type="button"
                  onClick={() => setStreakCalendarOpen(true)}
                  className="flex cursor-pointer items-center gap-1 rounded-full border border-orange-200/60 bg-orange-50/80 px-2.5 py-1 text-xs font-semibold text-orange-600 transition-colors hover:bg-orange-100 dark:border-orange-500/25 dark:bg-orange-500/10 dark:text-orange-300 dark:hover:bg-orange-500/15"
              ...
              {user ? (
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="btn-interactive flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--bg-elevated)] pl-1.5 pr-3 py-1.5 text-sm font-medium text-[var(--text-primary)] transition-all hover:bg-[var(--accent-bg)]"
                  >
                ...
                <div className="flex items-center gap-2">
                  <Link
                    href="/sign-in"
                    className="nav-auth-btn rounded-full px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:text-slate-900 dark:text-white hover:bg-slate-100"
                  >
                    {t.auth.signIn}
                  </Link>
                  <Link href="/sign-up" className="nav-auth-btn">
            ...
            {mobileMenuOpen && (
              ...
              >
                <div
                  ...
                  className="rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] shadow-[var(--shadow-lg)] backdrop-blur-[16px]"
                  style={{
                    paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))",
                  }}
                >
                  <nav className="flex max-h-[min(70vh,520px)] flex-col overflow-y-auto overscroll-contain px-3 py-3">
                    ...
                    <div className="mt-4 border-t border-[var(--border)] pt-4">
                      {/* Zinverts coins + streak row */}
                      <div className="mb-3 flex items-center gap-2 px-2">
                        <Link
                          href="/ai"
                          onClick={() => setMobileMenuOpen(false)}
                          className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-amber-200/60 bg-amber-50/80 py-2 text-sm font-semibold text-amber-700 dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-300"
                        >
                        ...
                        {streak > 0 && (
                          <button
                            ...
                            className="flex items-center gap-1 rounded-xl border border-orange-200/60 bg-orange-50/80 px-3 py-2 text-sm font-semibold text-orange-600 dark:border-orange-500/25 dark:bg-orange-500/10 dark:text-orange-300"
                          >
                      ...
                      <button
                        ...
                        className="btn-interactive mb-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[var(--border)] px-4 text-sm font-medium text-[var(--text-secondary)]"
                      >
                      ...
                      {user ? (
                        ...
                        <div className="flex flex-col gap-3 px-2">
                          <Link
                            href="/sign-in"
                            onClick={() => setMobileMenuOpen(false)}
                            className="nav-auth-btn flex min-h-12 items-center justify-center rounded-full border border-slate-200 bg-white text-[15px] font-medium text-slate-800 transition-colors active:bg-slate-100 dark:border-white/15 dark:bg-[#0a0f1c]/5 dark:text-white dark:active:bg-[#0a0f1c]/10"
                          >
                            {t.auth.signIn}
                          </Link>
                          <Link
                            href="/sign-up"
                            onClick={() => setMobileMenuOpen(false)}
                            className="nav-auth-btn flex min-h-12 items-center justify-center rounded-full bg-blue-500 text-[15px] font-semibold text-white transition-colors active:bg-blue-600"
                          >
                            {t.auth.signUp}
                          </Link>
                        </div>
