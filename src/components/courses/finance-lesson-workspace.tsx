...
import { ArrowLeft, Mic, Paperclip, PanelRightOpen, Send, StickyNote } from "lucide-react";
...
export function FinanceLessonWorkspace() {
  ...
  const [leftSize, setLeftSize] = useState(64);
  ...
  const editorScroll = (
    <div
      className={cn(
        ...
        assistantOpen
          ? "h-full min-h-0 overflow-y-auto border-r border-[var(--border)]"
          : "min-h-0 w-full",
      )}
    >
  ...
  return (
    <AuthGate>
      <div className="min-h-dvh bg-[var(--bg-primary)] text-[var(--text-primary)]">
        <Navigation />
        <div className="flex min-h-[calc(100dvh-0px)] flex-col pt-20 md:pt-24">
          ...
          {assistantOpen ? (
            <ResizablePanelGroup
              ...
              onLayout={(sizes) => {
                if (sizes[0] != null) setLeftSize(Math.round(sizes[0]));
              }}
            >
              <ResizablePanel defaultSize={leftSize} minSize={40} className="min-h-0 min-w-0">
                {editorScroll}
              </ResizablePanel>
              <ResizableHandle className="w-1.5 bg-[var(--border)]" />
              <ResizablePanel defaultSize={100 - leftSize} minSize={24} className="min-h-0 min-w-0">
                <aside className="flex h-full min-h-0 min-w-[280px] flex-col border-l border-[var(--border)] bg-[var(--bg-primary)]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">{editorScroll}</div>
          )}
          ...
          {!assistantOpen ? (
            <button
              ...
              className="fixed right-0 top-1/2 z-40 flex h-32 w-8 -translate-y-1/2 items-center justify-center rounded-l-lg border border-r-0 border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-muted)] shadow-lg transition-colors hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)]"
            >
